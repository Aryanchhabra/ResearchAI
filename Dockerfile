...
FROM install-browser AS research-ai-install
...
FROM research-ai-install AS research-ai
...
RUN useradd -ms /bin/bash research-ai && \
    chown -R research-ai:research-ai /usr/src/app && \
    # Add these lines to create and set permissions for outputs directory
    mkdir -p /usr/src/app/outputs && \
    chown -R research-ai:research-ai /usr/src/app/outputs && \
    chmod 777 /usr/src/app/outputs
USER research-ai
WORKDIR /usr/src/app

# Copy the rest of the application files with proper ownership
COPY --chown=research-ai:research-ai ./ ./
